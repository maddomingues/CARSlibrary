...
weightPoF.run.exp.cf <- function(dataFile='dataset.csv', resultFile='result_4.csv', neigh=4, fold=1) {
	...
	dimensions.names <<- c(tolower(names(res_pre)),'filterPoF')
	data <- weightPoF.load.data.and.setup(res_pre, fold)

	result <- NULL
	result <- weightPoF.cf.exp(Train=unique(data$train[,c(1,4)]),Test=unique(data$test[,c(1,4)]),Test.ids=data$test.ids,Hidden=data$hidden,Table='filterPoF',Field='filterPoF',Ntop=c(1,2,3,5,10),Neib=neigh)
...
weightPoF.post.filtering.weight <- function(User, B){
	...
	if(length(user.item.in.context) > 0){
		...
		for(i in 1:length(B)){
			...
			prob <- (item.neighbor / all.neighbor)
			if(prob < threshold){
				B[i] <- 0
			}
		}
